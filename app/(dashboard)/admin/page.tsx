  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      ...
      try {
        ...
        const semuaUser = usersSnap.docs.map(d => ({ id: d.id, ...d.data() }));
        const daftarGuru = semuaUser.filter((u: any) => u.role !== "admin");

        const guruTerbaru = daftarGuru
          .sort((a: any, b: any) => {
            const waktuA = a.dibuat_pada?.seconds || 0;
            const waktuB = b.dibuat_pada?.seconds || 0;
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-10">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-slate-100 pb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-2 font-medium">Selamat datang kembali, <span className="font-bold text-indigo-600">{adminName}</span>.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="p-6 rounded-3xl bg-white border border-slate-100 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="p-6 rounded-3xl bg-white border border-slate-100 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-sm mt-8">
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <h2 className="text-lg font-bold text-slate-800">Pendaftar Terbaru</h2>
          <Link href="/admin/users" className="text-sm font-bold text-indigo-600 hover:text-indigo-800 transition-colors">Lihat Semua Guru &rarr;</Link>
        </div>
            ...
            <tbody className="text-sm divide-y divide-slate-50">
              {recentUsers.map((u) => (
                <tr key={u.id} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="p-4">
                    <span className={`px-3 py-1.5 rounded-lg text-xs font-bold ${u.sisa_kuota < 10 ? 'bg-red-50 text-red-600' : 'bg-emerald-50 text-emerald-600'}`}>{u.sisa_kuota} Token</span>
                  </td>
                  <td className="p-4 pr-6 text-right">
                    <Link href="/admin/users" className="inline-block px-4 py-2 bg-slate-100 text-slate-600 group-hover:bg-indigo-50 group-hover:text-indigo-600 rounded-lg font-bold text-xs transition-colors">Kelola</Link>
                  </td>
                </tr>
              ))}
              {recentUsers.length === 0 && (
                <tr>
                  <td colSpan={4} className="p-10 text-center text-slate-500 italic font-medium">Belum ada guru yang mendaftar.</td>
                </tr>
              )}
            </tbody>
          </table>
